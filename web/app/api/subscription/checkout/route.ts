...
export async function POST(request: NextRequest) {
  ...
  console.log('- POLAR_ACCESS_TOKEN length:', POLAR_ACCESS_TOKEN?.length || 0);
  console.log('- POLAR_ACCESS_TOKEN prefix:', POLAR_ACCESS_TOKEN?.substring(0, 15) + '...');
  console.log('- POLAR_PRO_PRODUCT_ID:', process.env.POLAR_PRO_PRODUCT_ID || 'NOT SET');
  console.log('- POLAR_PREMIUM_PRODUCT_ID:', process.env.POLAR_PREMIUM_PRODUCT_ID || 'NOT SET');
  ...
  try {
    ...
    const checkoutPayload = {
      products: [productId],
      success_url: `${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/profile?subscription=success`,
    ...
    console.log('=== POLAR API RESPONSE ===');
