...
function convertMarkdownToHtml(markdown: string): string {
  ...
  html = html.replace(/```(\w*)\n?([\s\S]*?)```/g, (_, lang, code) => {
    ...
    codeBlocks.push(`<pre class="bg-gray-800 text-gray-100 p-3 rounded-lg text-xs overflow-x-auto my-2 font-mono"><code>${code.trim()}</code></pre>`)
    return placeholder
  })
  ...
  // Handle inline code
  html = html.replace(/`([^`\n]+)`/g, '<code class="bg-neutral-700 px-1 py-0.5 rounded text-xs font-mono text-emerald-400">$1</code>')
