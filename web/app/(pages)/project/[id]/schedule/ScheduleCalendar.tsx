...
export default function ScheduleCalendar({ schedule, onUpdate }: { schedule: Schedule, onUpdate: () => void }) {
  ...
  const handleNextMonth = () => {
    const examDateObj = new Date(schedule.exam_date)
    const nextMonth = new Date(currentMonth.getFullYear(), currentMonth.getMonth() + 1)
    // Allow navigation if the next month's first day is before or equal to exam date month
    if (nextMonth.getFullYear() < examDateObj.getFullYear() || 
        (nextMonth.getFullYear() === examDateObj.getFullYear() && nextMonth.getMonth() <= examDateObj.getMonth())) {
      ...
      <div className="bg-white rounded-lg shadow p-6 mb-6">
        <div className="flex items-center justify-between mb-6">
          ...
          <button
            ...
            disabled={(() => {
              const nextMonth = new Date(currentMonth.getFullYear(), currentMonth.getMonth() + 1)
              return nextMonth.getFullYear() > examDate.getFullYear() || 
                     (nextMonth.getFullYear() === examDate.getFullYear() && nextMonth.getMonth() > examDate.getMonth())
            })()}
            ...
            <div className="p-4 bg-gray-50 rounded-lg border border-gray-200">
              <p className="text-sm text-gray-600 text-center">
                {task.status === 'understood' 
                  ? '✅ You marked this task as understood' 
                  : '⚠️ You marked this task as needing more work'}
              </p>
              {task.status === 'need_work' && (
                <p className="text-xs text-gray-500 text-center mt-1">
                  A revision task has been added to your schedule
                </p>
              )}
            </div>
            <button
              ...
              disabled={isUpdating}
              className="w-full py-3 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 disabled:opacity-50 transition-colors flex items-center justify-center space-x-2"
            >
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
